...
def Invert_diff_flat_output_GP(x, u, thrust_hover, ygp,kgp):
    #note yaw =0 is fixed
    ygp = ygp*kgp
    m = 35.89/1000
    g = 9.8
    ...
    pitch = atan2(beta1,beta2)*180/PI
    yawrate = 0
    a_temp = LA.norm([0,0,g]-x[2,:]+[ygp[0],ygp[1],ygp[2]])
    # acc g correspond to 49000, at thrust_hover
    thrust = int(a_temp/g*thrust_hover)
    # limit max thrust
    thrust = min(thrust, 2.2*thrust_hover)
    return roll,pitch,yawrate,thrust
...
if __name__ == '__main__':
    ...
    Tf[1] = 8
    pc[1][0,:] = [-.40, -0.40, -0.95]
    pc[2][0,:] = [-0.40, -0.40, -0.95]
    pc[3][0,:] = [-0.35, -1.41, -0.95]
    Tf[3] = 3
    pc[4][0,:] = [-.35, -1.41, -0.65]
    pc[5][0,:] = [-.35, -1.41, -0.65]
    Tf[5] = 3
    Tf[6] = 8
    pc[6][0,:] = [-.40, -0.40, -0.95]
    pc[7][0,:] = [-0.40, -0.40, -0.95]
    pc[8][0,:] = [-0.35, -1.41, -0.95]
    Tf[8] = 3
    pc[9][0,:] = [-.35, -1.41, -0.65]
    pc[10][0,:] = [-.35, -1.41, -0.65]
    Tf[10] = 3
    Tf[11] = 8
    pc[11][0,:] = [-.40, -0.40, -0.95]
    pc[12][0,:] = [-0.40, -0.40, -0.95]
    pc[13][0,:] = [-0.35, -1.41, -0.95]
    Tf[13] = 3
    pc[14][0,:] = [-.35, -1.41, -0.65]
    pc[15][0,:] = [-.35, -1.41, -0.65]
...
#-------------------------Actual control experiment--------------------#
    ...
    #Kb=np.asarray(acker(AA,bb,[-5.2,-5.4,-5.6]))
    Kb=np.asarray(acker(AA,bb,[-6.6,-6.8,-7.0]))
    #Kb=np.asarray(acker(AA,bb,[-8.2,-8.4,-8.6]))  # was using this value
    ...
    ttt = rospy.Time.now()
    t_real =0
    kgp = 0.40 #gp trust ratio
    sample_flag = True # only sample relevant region
    sample_index = [0, 1, 5, 6, 10, 11];
    while not (rospy.is_shutdown() or t_real*dt>t_total):
        tt0 = rospy.Time.now()
        if flag_done == 1 and kk<15:
            # Bezier Interpolation
            T = Tf[kk] #time to complete, also scaling factor
            if (kk == 4 or kk == 9 or kk == 14):
               #kgp = kgp + 0.1
               T = T+1
            if (kk in sample_index):
                sample_flag = True
            else:
                sample_flag = False
            #if kk == 4:
        ...
        try:
            ...
            for i in range(N):
               ...
               # update real state
                ...
                if t_real == 0:
                   ...
                   #x[i][2,:] = np.array([phat[i][2,0],phat[i][2,1],phat[i][2,2]]) 
                   #np.array([xpre[2],ypre[2],zpre[2]]) 
            ...
            # update GP model with xold ynew
            if t_real >=1 and sample_flag==True: #KF is started
                ...
                # make GP prediction with xnew, note xnew becomes xold in the next time step
                if t_real > 1:
                    ypredict, Vy = gp.Predict(xnew)
                    #limit change rate in Z axis
            ...
            for i in range(N):
                ...
                # compute u with GP
                roll,pitch,yawrate,thrust = Invert_diff_flat_output_GP(x[i], u[i], thrust_hover[i], ypredict, kgp)
                # send to quads
            ...
            ypred_hist[t_real] = ypredict
            flag_hist[t_real] = sample_flag
            ...
            break
    # save data
    f = open('cf1_3rd_data_09202017_PIDdrift_v6GPWind70kernel.pckl', 'w')
    pickle.dump([t_hist, p_hist, phat_hist, ptrack_hist, u_hist, uhat_hist, cmd_hist, cmdreal_hist, rpytrack_hist, imu_hist, gyro_hist, thrust_hover[i], ypred_hist, flag_hist], f)
    f.close()
