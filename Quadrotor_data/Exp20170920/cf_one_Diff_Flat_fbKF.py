...
class Kalman():
    def __init__(self, xinit):
        self.Q = np.array([[1e-8, 1e-12, 1e-12],[1e-12, 1e-8, 1e-12],[1e-12, 1e-12, 2e-3]]) # dynamic noise
    ...
    def update(self, newdata):
        ...
        # Kalman gain
        Kt = self.S.dot(M.transpose())*1/( M.dot(self.S).dot(M.transpose())+self.R )
        # update
        self.X = self.X + Kt*(newdata-M.dot(self.X))
        self.S = self.S - np.outer(Kt,M.dot(self.S))
        return self.X
...
if __name__ == '__main__':
    ...
    Tf = np.array([1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5])*1.0
...
#-------------------------Actual control experiment--------------------#
    ...
    #Kb=np.asarray(acker(AA,bb,[-6.6,-6.8,-7.0])) # works, err 0.2
    Kb=np.asarray(acker(AA,bb,[-12.2,-6.8,-2.6]))  # works when kbz set to 0
    Kb[0,2]=Kb[0,2]*1.0
    ...
    while not (rospy.is_shutdown() or t_real*dt>t_total):
        ...
        try:
            ...
            for i in range(N):
                ...
                if t_real == 0:
                   xKF = Kalman(np.array([ptemp[0],phat[i][1,0],phat[i][2,0]]))
                   yKF = Kalman(np.array([ptemp[1],phat[i][1,1],phat[i][2,1]]))
                   zKF = Kalman(np.array([ptemp[2],phat[i][1,2],phat[i][2,2]]))
                else:
                   xpre = xKF.update(np.array(ptemp[0]))
                   ypre = yKF.update(np.array(ptemp[1]))
                   zpre = zKF.update(np.array(ptemp[2]))
            ...
            for i in range(N):
                ...
                xnext[i] = x[i] + xd[i]*dt   
                #x[i] = xnext[i] # TODO: remove
                # Compute roll/pitch/yawrate/thrust
                roll,pitch,yawrate,thrust = Invert_diff_flat_output(xnext[i], u[i], thrust_hover[i])
                # send to quads
                cfs[i].goto(xnext[i][0,:])    # send setpoint
    ...
    for i in range(N):
        cfs[i].goto(xnext[i][0,:])    # send setpoint
        cfs[i].send_cmd_diff(0, 0, 0, thrust_hover[i])  # send feedforward term

    # save data
    f = open('cf1_Diff_Flat20170425fb01.pckl', 'w')
