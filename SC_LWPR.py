...
if __name__ == '__main__':
    ...
    model_xdd.init_alpha = 10*np.ones([9,9])
    model_xdd.meta = True

    model_ydd = LWPR(9,1)
    model_ydd.init_D = 20*np.eye(9)
    model_ydd.update_D = True
    model_ydd.init_alpha = 10*np.ones([9,9])
    model_ydd.meta = True

    model_zdd = LWPR(9,1)
    model_zdd.init_D = 20*np.eye(9)
    model_zdd.update_D = True
    model_zdd.init_alpha = 10*np.ones([9,9])
    model_zdd.meta = True
    ...
    # train model (small data sets must be presented mult. times in rand. order)
    for k in range(40):
